 ...
 * Handles all scheduled tasks in a single endpoint to avoid Vercel cron limits
 * Runs every 2 hours and determines which tasks to execute based on time
 */
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    console.log(`[${now.toISOString()}] Master scheduler running at ${hour}:${minute} UTC`);

    // Task 1: Process Campaigns (2:00 AM UTC daily)
    if (hour === 2 && minute < 30) {
    ...
    // Task 2: Cache Cleanup (3:00 AM UTC daily) 
    if (hour === 3 && minute < 30) {
    ...
    // Task 3: Follow-ups (9:00 AM UTC daily)
    if (hour === 9 && minute < 30) {
    ...
    // Maintenance Tasks (6:00 AM UTC daily)
    if (hour === 6 && minute < 30) {
    ...
    // If no tasks ran, just log the health check
    if (results.length === 0 && errors.length === 0) {
      console.log(`No scheduled tasks for ${hour}:${minute} UTC`);
      return res.status(200).json({
        message: 'Master scheduler healthy',
        time: now.toISOString(),
        nextTasks: getNextTaskSchedule(hour)
      });
    }
...
/**
 * Get schedule for next tasks
 */
function getNextTaskSchedule(currentHour: number) {
  const tasks = [
    { name: 'Process Campaigns', hour: 2 },
    { name: 'Cache Cleanup', hour: 3 },
    { name: 'Daily Maintenance', hour: 6 },
    { name: 'Follow-ups', hour: 9 }
  ];
  
  const nextTasks = tasks.filter(task => task.hour > currentHour);
  if (nextTasks.length === 0) {
    // Next day
    return tasks.map(task => ({ ...task, tomorrow: true }));
  }
